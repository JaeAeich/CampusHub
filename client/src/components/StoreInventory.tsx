...
function StoreInventory() {
  ...
  const handleSheetChange = async (files: FileList | null) => {
    if (!files || files.length === 0) {
      toast({
        variant: 'destructive',
        title: 'No file uploaded. Please try again',
      });
      return;
    }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (data) {
          ...
          const expectedDataTypes: string[] = [
            'string',
            'string',
            'string',
            'number',
          ...
          const sheetHeaders = sheetFirstRow[0] as string[];

          if (!expectedHeaders.every((header) => sheetHeaders.includes(header))) {
            toast({
              variant: 'destructive',
              title: 'Please check the header format and try again.',
            });
            return;
          }

          // Validate data types
          const sheetData: object[] = XLSX.utils.sheet_to_json(sheet, { header: expectedHeaders });

          const isValidData = sheetData.slice(1).every((row) =>
            expectedHeaders.every((header, index) => {
              // @ts-expect-error: row was not getting set. will anyways shift from xlsx to form.
              ...
              if (expectedDataTypes[index] === 'number') {
                ...
                return (typeof isValidNumber).toString() === expectedDataTypes[index];
              }
        ...
        toast({
          variant: 'destructive',
          title: `Product could not be added: ${error}`,
        });
      }
    };
        ...
        <h2 className="align-center text-darkgray text-smm md:text-lg">
          ...
          <Button className="ml-2 bg-accent hover:bg-accentDark rounded-full w-16 h-16 p-1">
            <input
              ...
              onChange={(e) => handleSheetChange(e.target.files)}
            />
          </Button>
        </h2>
        {errorProducts && (
